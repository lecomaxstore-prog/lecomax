    ...
    <article class="card" onclick="openProductPage('${p.id}')" style="cursor:pointer">
      ...
      <div class="card__body">
        <div style="display:flex; justify-content:space-between; align-items:start">
             ...
             <div class="card__rating" style="font-size:0.8rem; margin-top:16px;">⭐ ${p.rating.toFixed(1)}</div>
        </div>
        
        <div class="card__bottom" style="margin-top:12px; display:flex; justify-content:space-between; align-items:center">
            <div class="card__price">
                ${p.price} MAD
                ${p.old ? `<s style="font-size:0.85rem; color:var(--muted); font-weight:400; margin-left:6px">${p.old} MAD</s>` : ``}
            </div>
            <button class="btn btn--primary btn--small" onclick="event.stopPropagation(); addToCart('${p.id}'); showToast('Added to Cart')" style="padding:6px 12px; border-radius:99px;">
               Add
            </button>
        </div>
      </div>
